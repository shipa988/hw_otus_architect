...
func (i Interactor) VerifyUser(token string, tokenType string) (sessionId,userId string,err error) {
	...
	if ok && tk.Valid {
		...
		uuidgen, ok := claims[uuidKey].(string)
		if !ok {
			return "","", errors.Wrapf(errors.New("user uuidgen is absent"), errVerifyToken, token)
		}
		...
		dbUserID,isSignIn, err := i.userAuthRepo.IsSignIn(ctx, uuidgen)
		if err != nil {
			return "","", errors.Wrapf(err, errVerifyToken, token)
		}
		if !isSignIn {
		...
		return uuidgen,strconv.FormatUint(dbUserID,10),err
	}
